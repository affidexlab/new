-- DecaFlow Native KYC/KYB/Accreditation Provider Tables
...
CREATE TABLE IF NOT EXISTS kyc_applications (
  ...
  application_type VARCHAR(20) DEFAULT 'individual', -- individual | business
...
CREATE TABLE IF NOT EXISTS kyc_documents (
  ...
  document_hash VARCHAR(128), -- SHA256 for integrity verification
  expiry_date DATE,
  status VARCHAR(30) DEFAULT 'pending', -- pending | verified | rejected | expired
...
CREATE TABLE IF NOT EXISTS accreditation_claims (
  id SERIAL PRIMARY KEY,
  claim_id VARCHAR(64) UNIQUE NOT NULL,
  application_id VARCHAR(64) NOT NULL REFERENCES kyc_applications(application_id) ON DELETE CASCADE,
  accreditation_basis VARCHAR(50) NOT NULL, -- income_over_200k, net_worth_over_1m, etc.
...
CREATE INDEX IF NOT EXISTS idx_accreditation_claims_application ON accreditation_claims(application_id);
CREATE INDEX IF NOT EXISTS idx_accreditation_claims_status ON accreditation_claims(status);

-- KYC Review Audit Trail
CREATE TABLE IF NOT EXISTS kyc_review_audit (
  id SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_kyc_audit_application ON kyc_review_audit(application_id);
...
CREATE INDEX IF NOT EXISTS idx_liveness_application ON kyc_liveness_checks(application_id);
