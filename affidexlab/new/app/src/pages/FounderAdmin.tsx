...
export default function FounderAdmin() {
  ...
  return <Shell><div style={{ maxWidth: 1320, margin: "0 auto", padding: "2rem 1.25rem" }}>
    ...
    {activeTab === "customers" && <><div style={cardStyle}><h2>Create free founder test customer</h2><p style={{ color: "rgba(255,255,255,0.6)" }}>Creates comped active access across Verify, Compliance, Shield, Agents, Institutional/RWA, and Audit without touching NOWPayments.</p><FieldGrid><input style={inputStyle} placeholder="Test customer email" value={testAccess.email} onChange={e => setTestAccess({ ...testAccess, email: e.target.value })} /><input style={inputStyle} placeholder="Name" value={testAccess.name} onChange={e => setTestAccess({ ...testAccess, name: e.target.value })} /><input style={inputStyle} placeholder="Company name" value={testAccess.companyName} onChange={e => setTestAccess({ ...testAccess, companyName: e.target.value })} /></FieldGrid><button onClick={createTestAccess} style={{ ...buttonStyle, marginTop: "0.75rem" }}>Create free test customer access</button></div><Recent title="Customers, orgs, and enquiries" rows={customers} /></>}
