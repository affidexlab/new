...
const TEST_PRODUCTS = [
  { key: "verify", label: "Verify API" },
  { key: "compliance", label: "Compliance" },
  { key: "shield", label: "Shield" },
  { key: "agents", label: "Autopilot" },
  { key: "institutional", label: "Institutional / RWA" },
...
export default function FounderAdmin() {
  ...
  const [testAccess, setTestAccess] = useState({ email: "", name: "Founder Test User", companyName: "DecaFlow Founder Test Account", products: ["verify"] });
  ...
  const toggleTestProduct = (product: string) => setTestAccess(p => {
    const products = p.products.includes(product) ? p.products.filter(x => x !== product) : [...p.products, product];
    return { ...p, products: products.length ? products : [product] };
  });
  const createTestAccess = async () => {
    const data = await api("/v1/admin/test-access", { method: "POST", body: JSON.stringify(testAccess) });
    const productNames = TEST_PRODUCTS.filter(p => data.products?.includes(p.key)).map(p => p.label).join(", ");
    setCreatedSecret(`PRODUCT ACCESS:\n${productNames || testAccess.products.join(", ")}\n\nORG API KEY:\n${data.orgApiKey}${data.verifyApiKey ? `\n\nVERIFY API KEY:\n${data.verifyApiKey}` : ""}`);
    setTestAccess({ email: "", name: "Founder Test User", companyName: "DecaFlow Founder Test Account", products: ["verify"] });
    loadAll();
  };
  const createLoginLink = async () => { const data = await api("/v1/admin/org-login-link", { method: "POST", body: JSON.stringify({ email: loginLinkEmail }) }); setCreatedSecret(`ONE-TIME LOGIN LINK (15 min):\n${data.loginUrl}`); setLoginLinkEmail(""); };
  ...
  return <Shell><div style={{ maxWidth: 1320, margin: "0 auto", padding: "2rem 1.25rem" }}>
    ...
    {activeTab === "customers" && <><div style={cardStyle}><h2>Create free founder test customer</h2><p style={{ color: "rgba(255,255,255,0.6)" }}>Choose the product dashboards this test customer should fully access. If you select only Shield, the account behaves like a paid Shield customer; if you select only Verify, it behaves like a paid Verify customer.</p><FieldGrid><input style={inputStyle} placeholder="Test customer email" value={testAccess.email} onChange={e => setTestAccess({ ...testAccess, email: e.target.value })} /><input style={inputStyle} placeholder="Name" value={testAccess.name} onChange={e => setTestAccess({ ...testAccess, name: e.target.value })} /><input style={inputStyle} placeholder="Company name" value={testAccess.companyName} onChange={e => setTestAccess({ ...testAccess, companyName: e.target.value })} /></FieldGrid><div style={{ marginTop: "1rem" }}><div style={{ color: "rgba(255,255,255,0.65)", fontSize: "0.85rem", fontWeight: 800, marginBottom: "0.6rem" }}>Which product dashboard should this test account unlock?</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: "0.5rem" }}>{TEST_PRODUCTS.map(product => <label key={product.key} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.7rem 0.8rem", borderRadius: 12, background: testAccess.products.includes(product.key) ? "rgba(59,130,246,0.16)" : "rgba(255,255,255,0.04)", border: `1px solid ${testAccess.products.includes(product.key) ? "rgba(59,130,246,0.5)" : "rgba(255,255,255,0.1)"}`, cursor: "pointer" }}><input type="checkbox" checked={testAccess.products.includes(product.key)} onChange={() => toggleTestProduct(product.key)} />{product.label}</label>)}</div></div><button onClick={createTestAccess} style={{ ...buttonStyle, marginTop: "0.9rem" }}>Create selected test customer access</button></div><div style={cardStyle}><h2>Generate customer login link</h2><p style={{ color: "rgba(255,255,255,0.6)" }}>Mints a one-time /login link for any active customer account — works even if email delivery fails. Share only with the account owner.</p><FieldGrid><input style={inputStyle} placeholder="Customer account email" value={loginLinkEmail} onChange={e => setLoginLinkEmail(e.target.value)} /></FieldGrid><button onClick={createLoginLink} style={{ ...buttonStyle, marginTop: "0.75rem" }}>Generate login link</button></div><Recent title="Customers, orgs, and enquiries" rows={customers} /></>}
