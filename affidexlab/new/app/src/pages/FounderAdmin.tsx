...
export default function FounderAdmin() {
  ...
  const [testAccess, setTestAccess] = useState({ email: "", name: "Founder Test User", companyName: "DecaFlow Founder Test Account" });
  ...
  const createTestAccess = async () => { const data = await api("/v1/admin/test-access", { method: "POST", body: JSON.stringify(testAccess) }); setCreatedSecret(`ORG API KEY:\n${data.orgApiKey}\n\nVERIFY API KEY:\n${data.verifyApiKey}`); setTestAccess({ email: "", name: "Founder Test User", companyName: "DecaFlow Founder Test Account" }); loadAll(); };
  ...
  return <Shell><div style={{ maxWidth: 1320, margin: "0 auto", padding: "2rem 1.25rem" }}>
    ...
    {activeTab === "customers" && <><div style={cardStyle}><h2>Create free founder test customer</h2><p style={{ color: "rgba(255,255,255,0.6)" }}>Creates comped active access across Verify, Compliance, Shield, Agents, Institutional/RWA, and Audit without touching NOWPayments.</p><FieldGrid><input style={inputStyle} placeholder="Test customer email" value={testAccess.email} onChange={e => setTestAccess({ ...testAccess, email: e.target.value })} /><input style={inputStyle} placeholder="Name" value={testAccess.name} onChange={e => setTestAccess({ ...testAccess, name: e.target.value })} /><input style={inputStyle} placeholder="Company name" value={testAccess.companyName} onChange={e => setTestAccess({ ...testAccess, companyName: e.target.value })} /></FieldGrid><button onClick={createTestAccess} style={{ ...buttonStyle, marginTop: "0.75rem" }}>Create free test customer access</button></div><div style={cardStyle}><h2>Generate customer login link</h2><p style={{ color: "rgba(255,255,255,0.6)" }}>Mints a one-time /login link for any active customer account — works even if email delivery fails. Share only with the account owner.</p><FieldGrid><input style={inputStyle} placeholder="Customer account email" value={loginLinkEmail} onChange={e => setLoginLinkEmail(e.target.value)} /></FieldGrid><button onClick={createLoginLink} style={{ ...buttonStyle, marginTop: "0.75rem" }}>Generate login link</button></div><Recent title="Customers, orgs, and enquiries" rows={customers} /></>}
