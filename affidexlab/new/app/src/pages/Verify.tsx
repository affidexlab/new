...
const FEATURES = [
  ...
  { icon: "🔀", title: "Mixer & Tumbler Detection", desc: "Identifies funds that have passed through Tornado Cash, ChipMixer, Sinbad, and 40+ other known mixing services across all supported chains." },
  ...
  { icon: "📍", title: "Jurisdiction Risk Scoring", desc: "Identifies wallet activity concentrated in high-risk jurisdictions — North Korea, Iran, Russia, and other FATF grey-listed territories." },
...
const USE_CASES = [
  ...
  { icon: "🤝", title: "OTC Desks & Brokers", desc: "Screen counterparty wallets before large OTC transactions to avoid unknowingly handling proceeds of crime." },
];

const PLANS = [
  {
    name: "Developer", price: "$0", period: "/month", highlight: false, checks: "1,000 checks/month",
    features: ["1,000 wallet checks per month", "OFAC & UN sanctions screening", "Ethereum & Arbitrum only", "Basic risk score & flags", "Community support"],
  },
  {
    name: "Growth", price: "$299", period: "/month", highlight: false, checks: "50,000 checks/month",
    features: ["50,000 wallet checks per month", "Full global sanctions coverage", "All 7 supported chains", "Mixer & darknet detection", "3-hop graph analysis", "Audit trail & report IDs", "Webhook alerts", "Email support"],
  },
  {
    name: "Business", price: "$799", period: "/month", highlight: true, checks: "500,000 checks/month",
    features: ["500,000 wallet checks per month", "Everything in Growth", "5-hop graph analysis", "Jurisdiction risk scoring", "Bulk screening (batch API)", "Monthly compliance reports", "Custom risk thresholds", "Dedicated account manager", "Priority support (24hr SLA)"],
  },
  {
    name: "Enterprise", price: "Custom", period: "", highlight: false, checks: "Unlimited checks",
    features: ["Unlimited wallet checks", "Custom sanctions list integration", "Custom chain support", "Dedicated infrastructure", "SLA guarantees", "White-label API option", "On-premise deployment option", "Quarterly compliance review"],
  },
];
...
export default function Verify() {
  useEffect(() => {
    document.title = "DecaFlow Verify API — Wallet Screening & Sanctions Checking";
  }, []);
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    useCase: "", chains: [], monthlyChecks: "", plan: "", message: "",
  });
  ...
  const toggleChain = (c: string) => setFormData((p) => ({
    ...p, chains: p.chains.includes(c) ? p.chains.filter((x) => x !== c) : [...p.chains, c],
  }));
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setFormLoading(true);
    try {
      const res = await fetch(`${API_BASE}/v1/verify/enquiry`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...formData, source: "verify-page" }),
      });
  ...
  const runDemo = async () => {
    ...
    setDemoResult(null);
    try {
      const res = await fetch(`${API_BASE}/v1/verify/demo`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ address: demoAddress, chain: demoChain }),
      });
      if (res.ok) {
    ...
    setDemoResult({
      ...
      sanctionsMatch: score > 85,
      mixerExposure: score > 60 ? 0.34 : score > 30 ? 0.08 : 0,
      darknetExposure: score > 75 ? 0.12 : 0,
      jurisdictionRisk: score > 60 ? "HIGH" : score > 30 ? "MEDIUM" : "LOW",
      recommendation: score < 25 ? "APPROVE" : score < 60 ? "REVIEW" : "REJECT",
      flags: score < 25 ? [] : score < 60 ? ["Interaction with flagged exchange", "Moderate transaction velocity"] : ["OFAC watchlist proximity", "Mixer exposure detected", "High-risk jurisdiction activity"],
  ...
  const inp = (): React.CSSProperties => ({
    ...
    color: "#fff", fontSize: "0.875rem", outline: "none", boxSizing: "border-box", fontFamily: "inherit",
  });
    ...
    <div style={{ background: "#0A0E27", color: "#fff", minHeight: "100vh", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.25rem 2rem", borderBottom: "1px solid rgba(255,255,255,0.08)", position: "sticky", top: 0, background: "rgba(10,14,39,0.97)", backdropFilter: "blur(12px)", zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: "1.5rem", alignItems: "center", flexWrap: "wrap" }}>
          {[["Compliance", "/compliance", false], ["Security Audit", "/audit", false], ["Verify API", "/verify", true], ["Swap", "/app", false]].map(([l, h, a]) => (
            <a key={l as string} href={h as string} style={{ color: a ? "#8b5cf6" : "rgba(255,255,255,0.6)", textDecoration: "none", fontSize: "0.9rem", fontWeight: a ? 600 : 400 }}>{l as string}</a>
          ))}
          <button onClick={() => openForm("Developer")} style={{ background: "#8b5cf6", color: "#fff", padding: "0.5rem 1.25rem", borderRadius: "8px", border: "none", cursor: "pointer", fontSize: "0.875rem", fontWeight: 600 }}>
            Get Free API Key
          </button>
        </div>
      </nav>
      ...
      <section style={{ padding: "6rem 2rem 4rem", maxWidth: "1100px", margin: "0 auto", textAlign: "center" }}>
        ...
        <p style={{ fontSize: "1.1rem", color: "rgba(255,255,255,0.65)", maxWidth: "660px", margin: "0 auto 2.5rem", lineHeight: 1.7 }}>
          Instant wallet screening against global sanctions lists, mixer databases, darknet exposure records, and jurisdiction risk profiles — across 7 blockchain networks. One API. Sub-100ms.
        </p>
        <div style={{ display: "flex", gap: "1rem", justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => openForm("Developer")} style={{ background: "#8b5cf6", color: "#fff", padding: "0.875rem 2rem", borderRadius: "10px", border: "none", cursor: "pointer", fontSize: "1rem", fontWeight: 700 }}>
            Get Your Free API Key
          </button>
          <a href="#demo" style={{ background: "rgba(255,255,255,0.06)", color: "#fff", padding: "0.875rem 2rem", borderRadius: "10px", textDecoration: "none", fontSize: "1rem", fontWeight: 600, border: "1px solid rgba(255,255,255,0.12)" }}>
            Try Live Demo
          </a>
        </div>
        {/* Stats */}
        <div style={{ display: "flex", marginTop: "4rem", flexWrap: "wrap", background: "rgba(255,255,255,0.04)", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}>
          {[["7", "Blockchain networks"], ["50+", "Sanctions lists monitored"], ["<100ms", "API response time"], ["5 hops", "Graph analysis depth"], ["99.9%", "Uptime SLA"]].map(([v, l], i) => (
            <div key={i} style={{ flex: "1 1 140px", padding: "1.75rem 1rem", textAlign: "center", borderRight: i < 4 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
              <div style={{ fontSize: "1.8rem", fontWeight: 800, color: "#8b5cf6", letterSpacing: "-0.02em" }}>{v}</div>
      ...
      <section id="demo" style={{ padding: "4rem 2rem", maxWidth: "900px", margin: "0 auto" }}>
        <h2 style={{ fontSize: "1.75rem", fontWeight: 800, textAlign: "center", marginBottom: "0.5rem" }}>Try it now</h2>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(139,92,246,0.25)", borderRadius: "20px", padding: "2rem" }}>
          <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", marginBottom: "1rem" }}>
            <input type="text" placeholder="Enter wallet address (0x...)" value={demoAddress} onChange={(e) => setDemoAddress(e.target.value)} onKeyDown={(e) => e.key === "Enter" && runDemo()}
              style={{ flex: "1", padding: "0.875rem 1rem", borderRadius: "10px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", color: "#fff", fontSize: "0.9rem", outline: "none", minWidth: "220px", fontFamily: "monospace" }} />
            <select value={demoChain} onChange={(e) => setDemoChain(e.target.value)} style={{ padding: "0.875rem 1rem", borderRadius: "10px", background: "#1f2937", border: "1px solid rgba(255,255,255,0.12)", color: "#fff", fontSize: "0.9rem", outline: "none", cursor: "pointer" }}>
              {CHAINS.map((c) => <option key={c} value={c.toLowerCase().replace(/\s/g, "")} style={{ background: "#1f2937" }}>{c}</option>)}
            </select>
            <button onClick={runDemo} disabled={demoLoading || !demoAddress.trim()} style={{ background: "#8b5cf6", color: "#fff", padding: "0.875rem 1.5rem", borderRadius: "10px", border: "none", cursor: "pointer", fontWeight: 700, fontSize: "0.9rem", opacity: (demoLoading || !demoAddress.trim()) ? 0.5 : 1 }}>
          ...
          {demoResult && (
            <div style={{ marginTop: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", padding: "1.25rem", borderRadius: "12px", background: `${levelColor(demoResult.riskLevel)}10`, border: `1px solid ${levelColor(demoResult.riskLevel)}30`, marginBottom: "1rem", alignItems: "center" }}>
                <div style={{ width: 64, height: 64, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: `${levelColor(demoResult.riskLevel)}20`, border: `2px solid ${levelColor(demoResult.riskLevel)}`, fontSize: "1.25rem", fontWeight: 800, color: levelColor(demoResult.riskLevel) }}>
                  {demoResult.riskScore}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: "1.05rem" }}>
                    Risk Score: {demoResult.riskScore}/100 — <span style={{ color: levelColor(demoResult.riskLevel) }}>{demoResult.riskLevel} RISK</span>
                  </div>
                  <div style={{ display: "flex", gap: "1rem", marginTop: "0.4rem", flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: "0.8rem", color: "rgba(255,255,255,0.5)" }}>Report ID: <strong style={{ color: "rgba(255,255,255,0.7)", fontFamily: "monospace" }}>{demoResult.reportId}</strong></span>
                  </div>
                </div>
              </div>
              {/* Tabs */}
              ...
              {activeTab === "response" && (
                <div style={{ background: "#0D1117", borderRadius: "12px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)" }}>
                  <div style={{ padding: "0.6rem 1rem", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", gap: "0.4rem" }}>
                    ...
                    <span style={{ marginLeft: "0.5rem", color: "rgba(255,255,255,0.35)", fontSize: "0.75rem" }}>decaflow-verify · response.json</span>
                  </div>
              ...
              {activeTab === "fields" && (
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {[
                    ["riskScore", "number (0–100)", "Composite risk score. 0 = clean. 100 = critical."],
                    ["riskLevel", '"LOW"|"MEDIUM"|"HIGH"|"CRITICAL"', "Human-readable risk band."],
                    ["sanctionsMatch", "boolean", "True if address matches a sanctions list."],
                    ["mixerExposure", "number (0–1)", "Fraction of funds with traceable mixer origin."],
                    ["darknetExposure", "number (0–1)", "Fraction of funds linked to darknet services."],
                    ["jurisdictionRisk", '"LOW"|"MEDIUM"|"HIGH"', "Risk level of dominant jurisdictions."],
                    ["recommendation", '"APPROVE"|"REVIEW"|"REJECT"', "Suggested action for this wallet."],
                    ["reportId", "string", "Unique ID — reference in your audit trail."],
                    ...
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "160px 1fr", gap: "1rem", padding: "0.75rem", borderRadius: "8px", background: i % 2 === 0 ? "rgba(255,255,255,0.03)" : "transparent", fontSize: "0.8rem" }}>
                      <div>
                        <div style={{ fontFamily: "monospace", color: "#8b5cf6", fontWeight: 700 }}>{field}</div>
                        <div style={{ color: "rgba(255,255,255,0.35)", fontSize: "0.72rem", marginTop: "0.15rem" }}>{type}</div>
                      </div>
                      <div style={{ color: "rgba(255,255,255,0.6)", lineHeight: 1.5 }}>{desc}</div>
                    </div>
                  ))}
                </div>
              )}
      ...
      <section style={{ padding: "4rem 2rem", maxWidth: "900px", margin: "0 auto" }}>
        <h2 style={{ fontSize: "1.75rem", fontWeight: 800, textAlign: "center", marginBottom: "2rem" }}>Integrate in minutes</h2>
        <div style={{ background: "#0D1117", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", overflow: "hidden" }}>
          ...
          <pre style={{ margin: 0, padding: "1.75rem 1.5rem", fontSize: "0.83rem", lineHeight: 1.75, color: "#e2e8f0", overflowX: "auto" }}>
    ...
    break;
      ...
      <section style={{ padding: "5rem 2rem", background: "rgba(255,255,255,0.02)", borderTop: "1px solid rgba(255,255,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <h2 style={{ fontSize: "2rem", fontWeight: 800, textAlign: "center", marginBottom: "3rem" }}>What the Verify API screens for</h2>
      ...
      <section style={{ padding: "5rem 2rem", maxWidth: "1100px", margin: "0 auto" }}>
        <h2 style={{ fontSize: "2rem", fontWeight: 800, textAlign: "center", marginBottom: "3rem" }}>Who uses DecaFlow Verify</h2>
      ...
      <section style={{ padding: "5rem 2rem", background: "rgba(255,255,255,0.02)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          <h2 style={{ fontSize: "2rem", fontWeight: 800, textAlign: "center", marginBottom: "0.75rem" }}>Pricing</h2>
          <p style={{ color: "rgba(255,255,255,0.5)", textAlign: "center", marginBottom: "3rem", fontSize: "1rem" }}>Start free. Scale as you grow.</p>
            ...
            {PLANS.map((plan, i) => (
              <div key={i} style={{ background: plan.highlight ? "rgba(139,92,246,0.08)" : "rgba(255,255,255,0.03)", border: plan.highlight ? "1px solid rgba(139,92,246,0.4)" : "1px solid rgba(255,255,255,0.08)", borderRadius: "20px", padding: "1.75rem", position: "relative" }}>
                ...
                <button onClick={() => openForm(plan.name)} style={{ display: "block", width: "100%", padding: "0.8rem", borderRadius: "10px", fontWeight: 700, fontSize: "0.875rem", background: plan.highlight ? "#8b5cf6" : "rgba(255,255,255,0.07)", color: "#fff", border: plan.highlight ? "none" : "1px solid rgba(255,255,255,0.12)", cursor: "pointer" }}>
                  {plan.name === "Developer" ? "Get Free API Key" : plan.name === "Enterprise" ? "Contact Sales" : "Get Started"}
                </button>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: "6rem 2rem", textAlign: "center" }}>
        <h2 style={{ fontSize: "2.2rem", fontWeight: 800, marginBottom: "1rem" }}>Start verifying wallets today.</h2>
        <p style={{ color: "rgba(255,255,255,0.55)", fontSize: "1rem", maxWidth: "500px", margin: "0 auto 2.5rem", lineHeight: 1.7 }}>First 1,000 checks are free. No credit card required. Production-ready in minutes.</p>
        <button onClick={() => openForm("Developer")} style={{ background: "#8b5cf6", color: "#fff", padding: "1rem 2.5rem", borderRadius: "12px", border: "none", cursor: "pointer", fontSize: "1.05rem", fontWeight: 700 }}>
          Get Your Free API Key
        </button>
      </section>
          ...
          <div style={{ background: "#111827", border: "1px solid rgba(139,92,246,0.3)", borderRadius: "20px", width: "100%", maxWidth: "580px", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ padding: "1.75rem 2rem 1rem", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <h2 style={{ fontSize: "1.35rem", fontWeight: 800, marginBottom: "0.25rem" }}>
                  {formStep === "success" ? "You're in! 🚀" : `Verify API — ${selectedPlan} Plan`}
                </h2>
                {formStep !== "success" && <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.875rem", margin: 0 }}>We'll send your API key and integration guide within 24 hours.</p>}
              </div>
            ...
            {formStep === "success" ? (
              <div style={{ padding: "3rem 2rem", textAlign: "center" }}>
                ...
                <h3 style={{ fontSize: "1.25rem", fontWeight: 700, marginBottom: "0.75rem" }}>API key on the way!</h3>
                <p style={{ color: "rgba(255,255,255,0.6)", lineHeight: 1.7, marginBottom: "2rem" }}>
                  We'll send your API credentials and integration guide to <strong>{formData.email}</strong> within 24 hours. The Developer plan gives you 1,000 free checks to test the integration immediately.
                </p>
                <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", marginBottom: "2rem" }}>
                  npm package: <code style={{ color: "#8b5cf6" }}>@decaflow/verify</code>
                </p>
                <button onClick={closeForm} style={{ background: "#8b5cf6", color: "#fff", padding: "0.875rem 2rem", borderRadius: "10px", border: "none", cursor: "pointer", fontWeight: 700, fontSize: "0.95rem" }}>Close</button>
              </div>
            ) : (
              <form onSubmit={handleFormSubmit} style={{ padding: "1.75rem 2rem 2rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
                  <div>
                    <label style={{ display: "block", fontSize: "0.8rem", color: "rgba(255,255,255,0.6)", marginBottom: "0.4rem", fontWeight: 600 }}>Company / Project Name *</label>
                    <input required type="text" value={formData.companyName} onChange={(e) => setFormData((p) => ({ ...p, companyName: e.target.value }))} placeholder="e.g. Patricia Technologies" style={inp()} />
                  </div>
                  <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
                  <div>
                    ...
                    <input required type="email" value={formData.email} onChange={(e) => setFormData((p) => ({ ...p, email: e.target.value }))} placeholder="you@company.com" style={inp()} />
                  </div>
                  <div>
                    ...
                    <input type="text" value={formData.telegram} onChange={(e) => setFormData((p) => ({ ...p, telegram: e.target.value }))} placeholder="@handle or +234..." style={inp()} />
                  </div>
                </div>
                <div style={{ marginBottom: "1rem" }}>
                  <label style={{ display: "block", fontSize: "0.8rem", color: "rgba(255,255,255,0.6)", marginBottom: "0.4rem", fontWeight: 600 }}>Primary Use Case *</label>
                  <select required value={formData.useCase} onChange={(e) => setFormData((p) => ({ ...p, useCase: e.target.value }))} style={sel()}>
                    <option value="" disabled style={{ background: "#1f2937" }}>Select your use case</option>
                ...
                <div style={{ marginBottom: "1rem" }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                    {CHAINS.map((c) => (
                      <button key={c} type="button" onClick={() => toggleChain(c)} style={{ padding: "0.35rem 0.8rem", borderRadius: "100px", fontSize: "0.8rem", fontWeight: 600, cursor: "pointer", border: formData.chains.includes(c) ? "1px solid #8b5cf6" : "1px solid rgba(255,255,255,0.12)", background: formData.chains.includes(c) ? "rgba(139,92,246,0.2)" : "transparent", color: formData.chains.includes(c) ? "#C4B5FD" : "rgba(255,255,255,0.6)" }}>
                        {c}
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ marginBottom: "1rem" }}>
                  <label style={{ display: "block", fontSize: "0.8rem", color: "rgba(255,255,255,0.6)", marginBottom: "0.4rem", fontWeight: 600 }}>Estimated Monthly Wallet Checks</label>
                  <select value={formData.monthlyChecks} onChange={(e) => setFormData((p) => ({ ...p, monthlyChecks: e.target.value }))} style={sel()}>
                    <option value="" style={{ background: "#1f2937" }}>Select volume</option>
                ...
                <div style={{ marginBottom: "1.5rem" }}>
                  <label style={{ display: "block", fontSize: "0.8rem", color: "rgba(255,255,255,0.6)", marginBottom: "0.4rem", fontWeight: 600 }}>Tell us more (optional)</label>
                  <textarea value={formData.message} onChange={(e) => setFormData((p) => ({ ...p, message: e.target.value }))} placeholder="Any specific compliance requirements, integration questions, or context about your project..." rows={3} style={{ ...inp(), resize: "vertical" }} />
                </div>
                <button type="submit" disabled={formLoading} style={{ width: "100%", padding: "1rem", borderRadius: "10px", background: "#8b5cf6", color: "#fff", border: "none", cursor: "pointer", fontWeight: 700, fontSize: "1rem", opacity: formLoading ? 0.6 : 1 }}>
                  {formLoading ? "Submitting..." : selectedPlan === "Developer" ? "Get My Free API Key" : `Get Started — ${selectedPlan} Plan`}
                </button>
                <p style={{ textAlign: "center", color: "rgba(255,255,255,0.35)", fontSize: "0.75rem", marginTop: "0.75rem" }}>
                  🔒 No credit card required for the Developer plan. Cancel anytime on paid plans.
                </p>
              </form>
            )}
          </div>
