...
async function deployFullStack10() { return deployFullStack(10); }
async function deployFullStack2() { return deployFullStack(2); }
...
describe('RWAToken', function () {
  describe('mint / basic transfer compliance', function () {
    it('mint respects compliance (fails to an unverified wallet)', async function () {
      const { token, owner } = await loadFixture(deployFullStack10);
    ...
    it('mint succeeds to a verified, eligible wallet', async function () {
      const { token, identityRegistry, owner } = await loadFixture(deployFullStack10);
    ...
    it('a compliant transfer between two verified wallets succeeds and updates holder count', async function () {
      const { token, complianceRules, identityRegistry, owner } = await loadFixture(deployFullStack10);
    ...
    it('allows one-for-one holder replacement at maxHolders because the final holder count is unchanged', async function () {
      const { token, complianceRules, identityRegistry, owner } = await loadFixture(deployFullStack2);
      const [, , , alice, bob, charlie] = await ethers.getSigners();
      await verifyWallet(identityRegistry, owner, alice);
      await verifyWallet(identityRegistry, owner, bob);
      await verifyWallet(identityRegistry, owner, charlie);
      await token.connect(owner).mint(alice.address, 100n);
      await token.connect(owner).mint(bob.address, 100n);

      expect(await complianceRules.currentHolders()).to.equal(2n);
      await token.connect(alice).transfer(charlie.address, 100n);
      expect(await complianceRules.currentHolders()).to.equal(2n);
      expect(await token.balanceOf(alice.address)).to.equal(0n);
      expect(await token.balanceOf(charlie.address)).to.equal(100n);
    });
  });
  ...
  describe('forcedTransfer — old path, unchanged, still owner-key-dependent by design', function () {
    it('replays the Guardian-audit attack chain and confirms this path is still exploitable exactly as documented', async function () {
      ...
      await identityRegistry.connect(owner).setIdentity(attacker.address, true, true, DEFAULT_EVIDENCE_HASH);
  ...
  describe('emergencyForcedTransfer — CRITICAL finding fix', function () {
    it('reverts with EscrowNotSet before an escrow address is configured', async function () {
      const { token, identityRegistry, owner, emergencyCouncil } = await loadFixture(deployFullStack10);
    ...
    it('only emergencyCouncil can set the escrow address — owner cannot', async function () {
      const { token, owner, emergencyCouncil, escrow } = await loadFixture(deployFullStack10);
    ...
    it('escrow cannot be set to the zero address or the token contract itself', async function () {
      const { token, emergencyCouncil } = await loadFixture(deployFullStack10);
    ...
    it('only emergencyCouncil can call emergencyForcedTransfer — owner cannot', async function () {
      const { token, identityRegistry, owner, emergencyCouncil, escrow } = await loadFixture(deployFullStack10);
      const [, victim] = await ethers.getSigners();
      await verifyWallet(identityRegistry, owner, victim);
      await token.connect(owner).mint(victim.address, 1000n);
    ...
    it('CRITICAL SCENARIO: rescues funds to escrow even with IdentityRegistry actively hostile (owner compromised)', async function () {
      const { token, complianceRules, identityRegistry, owner, emergencyCouncil, escrow } = await loadFixture(deployFullStack10);
      const [, victim, attacker] = await ethers.getSigners();
      await verifyWallet(identityRegistry, owner, victim);
      await token.connect(owner).mint(victim.address, 1000n);
      await token.connect(emergencyCouncil).setEscrowAddress(escrow.address);
    ...
    it('cannot move funds from the zero address (no compliance-bypassing mint)', async function () {
      const { token, emergencyCouncil, escrow } = await loadFixture(deployFullStack10);
      await token.connect(emergencyCouncil).setEscrowAddress(escrow.address);
    ...
    it('can empty a holder into a new escrow at maxHolders because the net holder count is unchanged', async function () {
      const { token, complianceRules, identityRegistry, owner, emergencyCouncil, escrow } = await loadFixture(deployFullStack2);
      const [, , , alice, bob] = await ethers.getSigners();
      await verifyWallet(identityRegistry, owner, alice);
      await verifyWallet(identityRegistry, owner, bob);
      await token.connect(owner).mint(alice.address, 100n);
      ...
      expect(await complianceRules.currentHolders()).to.equal(2n);
      await token.connect(emergencyCouncil).emergencyForcedTransfer(alice.address, 100n, 'court order');
      expect(await complianceRules.currentHolders()).to.equal(2n);
      expect(await token.balanceOf(alice.address)).to.equal(0n);
      expect(await token.balanceOf(escrow.address)).to.equal(100n);
    });
  });

  describe('compliance rules replacement disabled — H-01 fix', function () {
    it('does not allow proposing, executing, or cancelling compliance-rule replacement', async function () {
      const { token, complianceRules, owner } = await loadFixture(deployFullStack10);
      await expect(token.connect(owner).proposeComplianceRules(await complianceRules.getAddress()))
        .to.be.revertedWithCustomError(token, 'ComplianceRulesUpdateDisabled');
      await expect(token.executeComplianceRulesUpdate())
        .to.be.revertedWithCustomError(token, 'ComplianceRulesUpdateDisabled');
      await expect(token.cancelPendingComplianceRules())
        .to.be.revertedWithCustomError(token, 'ComplianceRulesUpdateDisabled');
      expect(await token.complianceRules()).to.equal(await complianceRules.getAddress());
    });
  });
});
